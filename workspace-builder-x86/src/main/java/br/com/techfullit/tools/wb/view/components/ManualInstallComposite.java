...
/**
 * The Class ManualInstallComposite.
 */
public class ManualInstallComposite extends DefaultComposite {
	...
	/**
	 * Instantiates a new manual install composite.
	 *
	 * @param parent
	 *            the parent
	 * @param style
	 *            the style
	 */
	public ManualInstallComposite(Composite parent, int style) {
		super(parent, style);
		...
		lblTitle.setText("Instalar Manual de Ambiente");
		...
		lblAbaixoPossvel
				.setText("Selecione os programas para instalação do seu ambiente.");
	...
	private void confirmInstall() {
		...
		if (applicationsSelected.isEmpty()) {
			AppMessageBox.informationBox(getShell(), Constants.ATTENTION,
					"Selecione ao menos um item para avançar.");
			return;
		}

		SessionApplication.getInstance().put(
				SessionApplication.APPLICATIONS_SELECTED, applicationsSelected);
	...
	private boolean containKitInCurrentVersion(Application app) {
		try {
			...
			for (Application appCurrentVersion : currentVersion
					.getApplications()) {
	...
	private void populateTable() {
		try {
			HashMap<String, Application> kistAvailable = (HashMap<String, Application>) SessionApplication
					.getInstance().get(
							SessionApplication.APPLICAITONS_AVAILABLE);
